...
- runner 位置：src/starfish/native/bin/iec61850_simulator_server，
...
def probe_iec61850_mms_binary() -> tuple[bool, str]:
    ...
    try:
        ...
        return False, (
            f"iec61850_simulator_server binary 不存在: {runner_path}。"
    ...
    if st.st_size < 1024:
        return False, (
            f"iec61850_simulator_server binary 文件过小 ({st.st_size} bytes): {runner_path}"
        )

    if not os.access(runner_path, os.X_OK):
        return False, (
            f"iec61850_simulator_server binary 无执行权限: {runner_path}"
        )

    return True, f"iec61850_simulator_server binary 可用: {runner_path} ({st.st_size} bytes)"
