...
def probe_iec104_binary() -> tuple[bool, str]:
    ...
    if not runner_path.exists():
        return False, (
            f"iec104 simulator server 未编译: {runner_path}。"
            f"请在 src/starfish/native/ 下执行 CMake 构建，"
            f"或通过 WHALE_NATIVE_RUNNER_DIR 环境变量指定目录。"
        )

    if not os.access(runner_path, os.X_OK):
        return False, (
            f"iec104 simulator server 无执行权限: {runner_path}"
        )

    return True, f"iec104 simulator server 可用: {runner_path}"
