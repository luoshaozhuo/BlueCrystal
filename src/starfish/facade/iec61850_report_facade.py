...
def probe_iec61850_report_binary() -> tuple[bool, str]:
    ...
    # 检查 simulator server
    try:
        st_sim = os.stat(sim_path)
    except OSError:
        return False, (
            f"iec61850_simulator_server binary 不存在: {sim_path}。"
            f"请在 src/starfish/native/ 下执行 CMake 构建（依赖 libiec61850）。"
        )

    if st_sim.st_size < 1024:
        return False, (
            f"iec61850_simulator_server binary 文件过小 ({st_sim.st_size} bytes): {sim_path}"
        )

    if not os.access(sim_path, os.X_OK):
        return False, (
            f"iec61850_simulator_server binary 无执行权限: {sim_path}"
        )

    # 检查 report runner
    try:
        st_run = os.stat(runner_path)
    except OSError:
        return False, (
            f"iec61850_report_runner binary 不存在: {runner_path}。"
            f"请在 src/starfish/native/ 下执行 CMake 构建（依赖 libiec61850）。"
        )

    if st_run.st_size < 1024:
        return False, (
            f"iec61850_report_runner binary 文件过小 ({st_run.st_size} bytes): {runner_path}"
        )

    if not os.access(runner_path, os.X_OK):
        return False, (
            f"iec61850_report_runner binary 无执行权限: {runner_path}"
        )

    return True, (
        f"IEC61850 Report binaries 可用: simulator={sim_path} ({st_sim.st_size} bytes), "
        f"runner={runner_path} ({st_run.st_size} bytes)"
    )
