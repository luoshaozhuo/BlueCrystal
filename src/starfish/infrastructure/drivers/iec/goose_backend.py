"""Starfish IEC 61850 GOOSE 协议 backend —— environment-pending stub。
...
class GooseBackend:
    ...
    def start(self) -> None:
        """启动 GOOSE backend（in-memory stub）。
    ...
    def write(self, point_id: str, value: Any) -> None:
        ...
        raise UnsupportedOperation(
            "write",
            "GooseBackend.write 尚未实现，GOOSE 协议仅支持多播发布",
        )
    ...
    def report(self) -> dict[str, Any]:
        ...
        raise UnsupportedOperation(
            "report",
            "GooseBackend.report 尚未实现，"
            "待 L2 veth 网络环境就绪后实现",
        )


__all__ = ["GooseBackend", "probe_goose_binary"]
