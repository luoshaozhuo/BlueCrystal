"""Starfish IEC 61850 Sampled Values 协议 backend —— environment-pending stub。
...
def probe_sv_binary() -> tuple[bool, str]:
    ...
    binary_path = starfish_root / "infrastructure" / "native" / "bin" / "iec61850_sv_publisher_simulator"
...
class SvBackend:
    ...
    本 backend 以 in-memory 模式维护点位值，mode="environment-pending"，
    ...
    def write(self, point_id: str, value: Any) -> None:
        ...
        raise UnsupportedOperation(
            "write",
            "SvBackend.write 尚未实现，SV 协议仅支持多播发布采样值",
        )
    ...
    def report(self) -> dict[str, Any]:
        ...
        raise UnsupportedOperation(
            "report",
            "SvBackend.report 尚未实现，"
            "待 L2 veth + PTP 时间同步环境就绪后实现",
        )


__all__ = ["SvBackend", "probe_sv_binary"]
