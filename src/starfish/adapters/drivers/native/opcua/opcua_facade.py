...
- runner 位置：src/starfish/infrastructure/native/bin/open62541_source_simulator，
...
def resolve_open62541_runner_path() -> Path:
    ...
    优先级：
        ...
        2. 默认路径 src/starfish/infrastructure/native/bin/open62541_source_simulator。
    ...
    # 默认路径：相对于 src/starfish/infrastructure/native/bin/
    starfish_root = starfish_root_from(__file__)
    return starfish_root / "infrastructure" / "native" / "bin" / "open62541_source_simulator"
...
def probe_opcua_binary() -> tuple[bool, str]:
    ...
    if not available:
        return False, (
            f"{reason}。请在 src/starfish/infrastructure/native/ 下执行 CMake 构建，"
