...
- runner 位置：src/starfish/infrastructure/native/bin/。
...
def resolve_iec61850_report_runner_path() -> Path:
    ...
    优先级：
        ...
        2. 默认路径 src/starfish/infrastructure/native/bin/iec61850_report_runner。
    ...
    return starfish_root / "infrastructure" / "native" / "bin" / "iec61850_report_runner"
...
def probe_iec61850_report_binary() -> tuple[bool, str]:
    ...
    if not sim_ok:
        return False, (
            f"{sim_reason}。请在 src/starfish/infrastructure/native/ 下执行 CMake 构建（依赖 libiec61850）。"
        )
    ...
    if not run_ok:
        return False, (
            f"{run_reason}。请在 src/starfish/infrastructure/native/ 下执行 CMake 构建（依赖 libiec61850）。"
        )
