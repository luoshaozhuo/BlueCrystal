...
def probe_iec101_codec() -> tuple[bool, str]:
    ...
    检查 starfish.domain.protocols.iec101 包是否可导入及其关键组件完整性。
    ...
    try:
        ...
        if find_spec("starfish.domain.protocols.iec101") is None:
            return (False, "IEC101 编解码器包 (starfish.domain.protocols.iec101) 不存在")
        from starfish.domain.protocols.iec101 import (  # noqa: E402 动态导入
            ASDUHeader,
            decode_asdu_header,
            encode_asdu_header,
        )
...
def probe_iec101_codec_enhanced_plus() -> tuple[bool, str]:
    ...
    """
    try:
        from starfish.domain.protocols.iec101 import (  # noqa: E402
            CP56Time2a,
            C_SE_NA_1_Object,
            C_SE_NB_1_Object,
            C_SE_NC_1_Object,
        ...
        from starfish.domain.protocols.iec101 import SIQ
...
def probe_iec101_binary() -> tuple[bool, str]:
    ...
    native_bin = starfish_root / "infrastructure" / "native" / "bin"
...
class Iec101Facade:
    ...
    def mode(self) -> str:
        ...
        starfish_root = Path(__file__).resolve().parents[2]
        binary_path = starfish_root / "infrastructure" / "native" / "bin" / "iec101_simulator_slave"
