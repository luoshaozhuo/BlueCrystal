...
- runner 位置：src/starfish/adapters/protocols/iec104/native/bin/iec104_simulator_server，
...
from starfish.adapters.protocols.iec104.native.runtime import (
    native_runner_env,
    probe_native_binary,
    starfish_root_from,
)
...
def resolve_iec104_runner_path() -> Path:
    ...
    优先级：
        ...
        2. 默认路径 src/starfish/infrastructure/native/bin/iec104_simulator_server。
    ...
    if env_dir:
        ...
        base = starfish_root / "adapters" / "protocols" / "iec104" / "native" / "bin"
...
def probe_iec104_binary() -> tuple[bool, str]:
    ...
    if not available:
        return False, (
            f"{reason}。请构建 IEC104 native runner，"
...
class Iec104NativeBackend:
    ...
    unavailable 模式（binary 缺失）：
        ...
        仅保留生命周期和状态，以便明确暴露 native runner 不可用。
    ...
    def health(self) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            ...
            "definition_loaded": self._definition is not None,
            "point_count": len(self._definition.point_items) if self._definition else 0,
            "endpoint_count": 1 if self._definition else 0,
            "capabilities": list(self._definition.capabilities) if self._definition else [],
    ...
    def load_points(self, definition: ServerDefinition) -> None:
        ...
        Args:
            definition: 已从 DB view 映射完成的 IEC104 server definition。
        """
