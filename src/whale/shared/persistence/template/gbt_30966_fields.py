"""共享持久化层 GB/T 30966.2-2022 逻辑节点与字段定义。
...
本模块是 ``whale.shared.persistence.template`` 包内自持的真实参考数据，
不再 re-export ``seahorse.reference_data``，也未保留任何兼容 wrapper。
调用方直接 ``from whale.shared.persistence.template.gbt_30966_fields
import ALL_LOGICAL_NODES, ...`` 即可。
"""
