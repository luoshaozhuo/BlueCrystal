...
本文件是 ``whale.shared.persistence.template`` 子包内自持的真实参数定
义，定义“某协议某服务需要哪些参数”，不绑定具体 endpoint 或 signal。
正式参数应进入第一范式参数表；禁止把协议地址字段塞回主表，也禁止把
``metadata_json`` 当作正式参数主存储。

本模块不再 re-export ``seahorse.reference_data``，也未保留任何兼容
wrapper；调用方直接 ``from whale.shared.persistence.template.protocol_param_data
import ENDPOINT_PARAM_DEFS, SIGNAL_PARAM_DEFS, ...`` 即可。
"""
