"""共享持久化层 SCADA 协议样例数据生成器 —— whale 元数据库种子写入。

本文件负责生成一套共享的 SignalProfile/SignalProfileItem，并为 16 组
协议-服务样例写入 Organization/AssetType/AssetModel/CDCDict/FCDict/
ScadaDataType/ScadaProtocolParamDef/ScadaSignalParamDef/AssetInstance/
IED/CommunicationEndpoint/LDInstance/ScadaEndpointParamValue/
ScadaSignalProfileItemParamValue/AcquisitionTask。

样例数据生成与清理只触及 whale 元数据库，不依赖 seahorse runtime；
seahorse 侧通过 seahorse.infrastructure.repositories.whale_metadata_repository
的薄包装调用本文件的入口。
"""
...
def _load_protocol_param_defs() -> tuple[EndpointParamDefs, SignalParamDefs]:
    ...
    由 ``whale.shared.persistence.template`` 内部聚合真实协议参数表，
    由 Alembic 与 Whale 共享协议参数表统一管理；本模块不再使用任何
    已删除的兼容 wrapper。
    """
