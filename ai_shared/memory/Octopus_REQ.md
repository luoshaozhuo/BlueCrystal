...
本文件描述 Octopus 运维执行面需求。Octopus 与 BlueCrystal 并列，负责运维观测、统一部署编排、监控、告警、诊断执行、自动化恢复、回滚和运行报告。
...
| P-AR-003 | 承接 Octopus 与 BlueCrystal/Turtle/PlatformShared 的边界 |
...
- 优先级：高
- 需求描述：
  - Octopus 应统一编排 BlueCrystal、Turtle、Dolphin、Jellyfish、Manta 等组件的部署、升级、回滚和健康检查。
- 验收要点：
  - 读取各模块自己的部署定义。
  - 不接管单模块部署定义。
  - 部署准入由 Turtle 判断。
...
### OC-AR-001 Octopus 与 BlueCrystal 并列
...
| OC-AR-001 | P-AR-003 | Octopus 与 BlueCrystal 并列 | AR | 高 | octopus | L3 | 已验证 | src/octopus 与 src/whale 并列；boundary test 升级至 79 tests 覆盖 platform_shared 全量边界 | import boundary tests (79 passed) | 无 | 无 | 2026-06-02 |
