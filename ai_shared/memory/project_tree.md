...
├── docker-compose.whale-l5.yaml     — L5 外部依赖 5-service Docker 环境（Kafka/PG/Redis/MinIO/TDengine）
...
│   ├── __init__.py                   — 包入口
│   ├── raw_archive.py                — 压缩文件归档（S3RawArchiveSink boto3 +gzip JSONL + LocalCompressedArchiveSink + S3ManifestRepository）
...
└── shared/                           — 共享层（跨模块通用能力）
    ...
    │       └── stagger.py            — 错峰调度
    │
    └── crosscutting/                 — 横切关注点（保留旧路径兼容）
        ├── __init__.py
        ├── auth/                     — 认证授权（shim，实现在 turtle）
        │   ├── __init__.py
        │   ├── authorizer.py
        │   ├── credential.py
        │   ├── identity.py
        │   └── policy.py
        ├── compliance/              — 合规（shim，实现在 turtle）
        │   ├── __init__.py
        │   ├── audit_policy.py
        │   ├── data_classification.py
        │   └── retention.py
        ├── security/                — 安全（shim，实现在 turtle）
        │   ├── __init__.py
        │   ├── certificate.py
        │   ├── model.py
        │   ├── secret_provider.py
        │   └── tls.py
        ├── debug/                   — 调试与诊断（shim，实现在 platform_shared）
        │   ├── __init__.py
        │   ├── diagnostics.py
        │   ├── ring_buffer.py
        │   └── trace.py
        ├── observability/           — 可观测性（shim，实现在 platform_shared）
        │   ├── __init__.py
        │   ├── audit.py
        │   ├── logging.py
        │   ├── masking.py
        │   └── metrics.py
        └── resilience/              — 韧性策略（shim，实现在 platform_shared）
            ├── __init__.py
            ├── backoff.py
            ├── circuit_breaker.py
            ├── deadline.py
...
│
├── security/                        — 安全基础（从 crosscutting 迁入）
│   ├── __init__.py
...
│
├── compliance/                      — 合规基础（从 crosscutting 迁入）
│   ├── __init__.py
...
│   ├── test_mqtt_backend.py            — MQTT client backend 单测（L1 unit/mock，asyncio MQTT v3.1.1）
│   ├── test_mqtt_source_acquisition_adapter.py — MQTT 采集适配器单测（L1 unit/mock）
│   ├── test_http_rest_backend.py       — HTTP REST client backend 单测（L1 unit/mock）
│   ├── test_http_rest_source_acquisition_adapter.py — HTTP REST 采集适配器单测（L1 unit/mock）
│   ├── test_iec104_backend.py           — IEC104 lib60870 backend 单测
│   ├── test_iec104_source_acquisition_adapter.py   — IEC104 采集适配器单测（L1 unit/mock）
│   ├── test_iec104_source_write_adapter.py          — IEC104 写入适配器单测
│   ├── test_modbus_rtu_backend.py       — Modbus RTU serial backend 单测（L1 unit/mock，CRC16）
│   ├── test_modbus_rtu_source_acquisition_adapter.py — Modbus RTU 采集适配器单测（L1 unit/mock）
│   ├── test_iec101_backend.py           — IEC101 serial backend 单测（L1 unit/mock，FT1.2+ASDU）
│   ├── test_iec101_source_acquisition_adapter.py   — IEC101 采集适配器单测（L1 unit/mock）
...
│   ├── test_http_rest_acquisition_chain.py — HTTP REST 全链路采集集成测试（L3 simulator）
│   ├── test_iec104_acquisition_chain.py    — IEC104 全链路采集集成测试（L3 simulator）
│   ├── test_modbus_rtu_acquisition_chain.py — Modbus RTU 全链路采集集成测试（L3 simulator）
│   ├── test_iec101_acquisition_chain.py    — IEC101 全链路采集集成测试（L3 simulator）
│   ├── test_mqtt_acquisition_chain.py      — MQTT 全链路采集集成测试（L3 simulator）
...
│   ├── conftest.py
│   ├── helpers.py
│   ├── test_whale_field_minimal_smoke.py — Whale 现场最小数据链路 L4 smoke（7 tests）
│   ├── test_whale_l5_kafka_pipeline_e2e.py — L5 Kafka pipeline E2E（4 tests）
│   └── test_whale_l5_storage_e2e.py        — L5 storage E2E（10 tests：S3/TDengine/Redis）
│
├── performance/                       — 性能测试
│   ├── __init__.py
│   ├── load/
