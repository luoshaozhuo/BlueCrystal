...
### I-FR-014 文件接入 Adapter

- 类型：功能
- 优先级：中
- 需求描述：
  - ingest 模块应支持从文件系统接入故障录波、事件记录等离线/准实时文件数据。
  - 文件接入应包含完成检测、归档、解码、波形写入和故障事件记录。
- 验收要点：
  - 支持文件完成检测（inotify + 轮询 fallback）。
  - 支持 raw_archive 归档（底层复用 storage.raw_archive）。
  - 支持二进制解码（FaultRecordBinaryDecoder：magic + version + header + float32 LE values）。
  - 解码输出 DecodedSignal + StandardizedWaveformValue，不直接写 standardized 点值表。
  - 波形独立 sink（StandardizedWaveformSinkPort），与 TdengineStandardizedSink（点值）分离。
  - 故障事件（fault_event）记录为运行期元数据，ORM 模型留到后续。
  - 文件接入不得依赖 source_lab。
...
### I-TEST-003 文件接入开发期/模块集成期验证

- 类型：测试与验收
- 优先级：中
- 需求描述：
  - file_ingest 子系统必须具备 unit 和 integration 测试，覆盖文件检测、解码、归档、波形写入和编排闭环。
- 验收要点：
  - unit 测试覆盖 FaultRecordBinary、FileCompletionDetector、FaultRecordBinaryDecoder、FileIngestJobRepository、FileIngestService。
  - integration 测试覆盖 detect->raw_archive->decode->waveform sink->fault_event 闭环。
  - decode-before-resolve 约束保持：decoder 输出 DecodedSignal + StandardizedWaveformValue，不直接 resolve。
  - 波形 sink 的 TdengineStandardizedWaveformSink 已实现为真实 REST API adapter（write()/readback()），当前环境 taosAdapter REST API 不可用（TCP 可达但 /rest/sql 返回 404），P5 集成测试 4 tests FAIL（MISSING_ENVIRONMENT，非代码缺陷）。正确行为：连接失败时不抛异常，write() 返回 False，readback() 返回 []。
  - skipped 不得作为完成证据。
...
- 验收要点：
  ...
  - P3 contract、mock、simulator 证据不等于真实设备投产证据。
...
| I-READY-005 | P-SCR-001 | ingest 写入控制投产准入 | READY | 高 | ingest_write_runtime | P3/P4 | 部分实现 | `scripts/run_ingest_write_readback_smoke.sh`; `tests/integration/test_ingest_opcua_source_write.py`; `tests/integration/test_ingest_modbus_source_write.py`; `tests/integration/test_ingest_iec61850_mms_source_write.py`; `scripts/test_ingest_write_readback_smoke_contract.sh`（field_readback 相关报告文件已在 Round 4 清理归档）| Round 16: 脚本 CLI 加固（--dry-run/--protocol/--confirm/--write-enabled/--audit-output/--evidence-report/--help），protocol 别名支持，evidence report 生成修复，入口 smoke 自检 10/10 PASS (P3 contract)；Round 13: WRITE_ENABLED=false、CONFIRM_FLAG=false 双重安全门确认正确；三协议 P3 readback contract 各 3 tests；真实写入默认关闭，dry-run 安全；无 P5 伪造 | 真实设备 / 真实网关 / 真实授权链路的 P5 readback 仍缺失 -- **唯一生产准入硬阻塞** | 按 field plan + evidence template 执行现场验证与回滚归档 | 2026-06-03 |
| I-READY-006 | P-NFR-002 | ingest 多节点生产准入 | READY | 高 | ingest_scheduler | P6 | 已实现（注1） | `tests/integration/test_ingest_dual_node_db_lease_e2e.py`; `tests/integration/test_ingest_prodlike_postgres_fault_injection.py`; `scripts/run_ingest_pg_lease_fault_injection.sh`; `docker-compose.ingest-prodlike.yaml`; `src/whale/ingest/runtime/fencing.py` | Round 15: SQLite P4 dual-node lease E2E 7/7 passed；PG P6 4/4 passed（lease_acquire/release/concurrent_read/fencing_token 并发 INSERT ON CONFLICT DO UPDATE RETURNING 原子操作）；fencing_token race 已修复：IntegrityError → LEASE_CONFLICT；DB fault injection 通过；证据等级从 P6 partial 提升为 P6 complete | 真实现场多节点跨主机部署未验证；网络分区与旧主恢复 E2E 未收口 | 补真实现场多节点跨主机部署验证；补网络分区与旧主恢复 E2E | 2026-05-31 |
| I-READY-007 | P-NFR-004 | ingest 质量门禁 | READY | 高 | ingest_quality | P4 | 已实现 | `ai_shared/reports/source_lab_mypy_phase1_closure_round13.md`; `ai_shared/reports/source_lab_mypy_phase2_closure_round14.md`; `ai_shared/reports/source_lab_tests_mypy_closure_round15.md`; `tests/unit/test_ingest_no_source_lab_imports.py` | Round 15: source_lab 全量 mypy 0 errors / 202 files（cmd/src/tests 全覆盖）；compileall PASS；ruff PASS；mypy src/whale/ingest src/whale/shared/source PASS；import boundary PASS；ingest 自身质量门禁已收口，source_lab mypy 治理终点已达成 | 无；mypy 全量清零，质量门禁全部收口 | 维持 mypy gate CI，防止类型退化 | 2026-05-31 |
| I-READY-008 | P-SCR-001/P-FR-003 | ingest 现场验证准备就绪 | READY | 高 | ingest_write_runtime | P3 | 已实现 | `scripts/run_ingest_write_readback_smoke.sh`（CLI 加固：--dry-run/--protocol/--confirm/双安全门）；`scripts/test_ingest_write_readback_smoke_contract.sh`（10/10 PASS；field_readback 报告文件已在 Round 4 清理归档） | Round 16: 写入回读脚本 CLI 接口契约完整，P5 现场证据模板就绪，入口自检全部通过，dry-run 默认安全，CONFIRM 双安全门正确，审计/证据报告可生成，无凭据泄露 | 无 -- 现场验证包已准备完毕，等待真实设备/网关环境执行 P5 field readback | 现场团队按 evidence template 执行三协议 write-readback 并填写 P5 证据 | 2026-06-03 |
...
| 编号 | 项 | 证据等级 | 状态 |
|---|---|---|---|
| I-READY-006 | 多节点生产准入（lease/fencing） | P6 | 已实现 -- PG P6 4/4，fencing_token race 已修复 |
| I-READY-007 | 质量门禁 | P4 | **已实现** -- mypy production+tools 0 errors/402 files（types-PyYAML 修复 yaml stubs），compileall/ruff 全量通过，import boundary PASS；mypy tests 281 历史债务单独归类，不阻塞 production gate |
| I-READY-008 | 现场验证准备就绪 | P3 | **已实现** -- P5 外部依赖环境验证准备就绪（`ai_shared/field_readback/` 执行包已删除，P5 定义从"现场环境验证"修正为"准生产真实外部依赖环境验证"），环境预检脚本就绪（`scripts/check_l5_field_readback_env.py`、`scripts/check_serial_env.py`、`scripts/check_ads_env.py`、`scripts/check_l2_goose_sv_env.py`），CI 质量门禁聚合脚本就绪（`scripts/run_quality_gate.py`）；真实设备/网关环境 P5 验证为 env-pending（不在本项目需求跟踪表验证等级范围内） |
| I-READY-002 | 外部依赖准入矩阵 | P6 | 部分实现 -- compose readyz 8/8 PASS |
| I-READY-003 | 横切能力接入准入 | P4 | 接入完毕 -- 8/8 crosscutting matrix |
| I-READY-001 | 独立部署准入 | P4 | 部分实现 -- image/entrypoint/compose smoke |
| I-READY-004 | 安全分区部署约束 | P3/P4 | 部分实现 -- 部署拓扑/端口矩阵/单向链路 |
| - | **IEC104 production-ready** | P4 | **已实现** -- DispatchSourceAcquisitionAdapter 接入 PollingAcquisitionRole，write 4 E2E，readiness gate 已列入，PROTOCOL_CAPABILITIES write=True |
| - | **MQTT acquisition-ready** | P4 | **已实现** -- asyncio MQTT v3.1.1 backend + adapter + 3 E2E，readiness gate 已列入 |
| - | **HTTP REST acquisition-ready** | P4 | **已实现** -- asyncio HTTP/1.1 backend + adapter + 4 E2E，readiness gate 已列入 |
| - | **Modbus RTU acquisition-ready** | P1-P4 | **已实现** -- real os/termios/fcntl serial backend（CRC16）+ adapter + E2E，read-only，write=NOT_IMPLEMENTED，readiness gate 已列入 |
| - | **IEC101 acquisition-ready** | P1-P4 | **已实现** -- real os/termios/fcntl serial backend（FT1.2+ASDU）+ adapter + E2E，read-only，write=NOT_IMPLEMENTED，readiness gate 已列入 |

### 协议生产就绪矩阵（Round 28）

| 协议 | acquisition | write | E2E | readiness gate | 状态 |
|---|---|---|---|---|---|
| OPC UA | production-ready | production-ready (P3 readback) | P4 | 已列入 | production-ready |
| Modbus TCP | production-ready | production-ready (P3 readback) | P4 | 已列入 | production-ready |
| IEC 61850 MMS | production-ready | production-ready (P3 readback) | P4 | 已列入 | production-ready |
| IEC 61850 Report | production-ready | NOT_IMPLEMENTED | P4 | 已列入 | production-ready |
| **IEC 104** | **production-ready** | **production-ready** (4 E2E) | **P4** (4 tests) | **已列入** | **production-ready** |
| **Modbus RTU** | **acquisition-ready** (read-only serial) | NOT_IMPLEMENTED | **P4** (serial read-only) | **已列入** | **acquisition-ready** (read-only serial; os/termios/fcntl; P6/P5 serial hardware=environment-pending) |
| **IEC 101** | **acquisition-ready** (read-only serial) | NOT_IMPLEMENTED | **P4** (serial read-only) | **已列入** | **acquisition-ready** (read-only serial; os/termios/fcntl; P6/P5 serial hardware=environment-pending) |
| MQTT | acquisition-ready | NOT_IMPLEMENTED | P4 (3 tests) | 已列入 | acquisition-ready |
| HTTP REST | acquisition-ready | NOT_IMPLEMENTED | P4 (4 tests) | 已列入 | acquisition-ready |
...
| 编号 | 项 | 当前证据 | 阻塞原因 |
|---|---|---|---|
| **I-READY-005** | **写入控制投产准入** | P3 contract + P4 simulator/native + P5 env-pending | **P5 外部依赖环境验证缺失 -- 5 协议（OPC UA/Modbus TCP/IEC 61850 MMS/IEC104/IEC 61850 Report）真实设备/网关环境未覆盖，不在本项目需求跟踪表验证等级范围内** |

### 最终判定（Round 28）

```
ingest 当前状态：prodlike-ready / production-ready blocked by P5 field readback + serial hardware env-pending

12 协议五维度六态矩阵已收口：
  - 5 production-ready (P5 pending): OPC UA, Modbus TCP, IEC 61850 MMS, IEC 61850 Report, IEC 104
...
所有 PG 级多节点机制（lease/fencing/assignment）已 P6 验证通过。
所有 compose 级依赖（readyz 8/8）已 P6 验证通过。
...
阻塞项（Round 4 更新）：
1. S3/TDengine/Redis/Pulsar/Flink/HDFS 准生产外部依赖环境未就绪 -- P5 env-pending。
2. Modbus RTU/IEC101 serial hardware P6/P5=environment-pending。
...
5. GOOSE/SV tool-ready-only（需受控 P3 环境 + CAP_NET_RAW）。
6. 五协议（OPC UA/Modbus TCP/IEC 61850 MMS/IEC104/IEC 61850 Report）真实设备/网关环境验证不在本项目需求跟踪表验证等级范围内。

在 P5 外部依赖环境 + serial hardware 验证完成并通过审计前，ingest 不得标 production-ready。
```

更新时间：2026-06-03 (Round 6 -- P5 定义修正、field_readback 删除、REQ 状态对齐、全模块 REQ 同步收口)
