...
> 当前工作区补充（2026-08-10）：本轮 Starfish 运行主链已收敛为 Hexagonal
> Architecture 下的 Whale view-driven Source simulator。通用分派只读
> `vw_comm_connection`；IEC104 只读 `vw_comm_iec104_connection`、
> `vw_src_point_table`、`vw_src_iec104_point_item`；ADS 只读
> `vw_comm_ads_connection`、`vw_src_point_table`、`vw_src_ads_point_item`。
> IEC104 已由真实 `c104` client 对当前 view connection 1/2 完成站总召与更新值
> 定向验证；ADS 已由真实 `pyads` client 对当前 view connection 3 完成同步读取、
> 3 个 CYCLIC 与 1 个 ON_CHANGE notification、4 次 DELETE，以及删除后无回调验证。
> ADS 的支持关闭契约限定为 `DELETE notification -> client close -> session cleanup
> -> server stop`；活动 callback 未 DELETE 时直接 teardown 可能触发 AdsLib
> SIGSEGV。上述证据是当前数据库与定向协议验证，不代表全仓回归、长稳、性能、
> 故障注入、现场 TwinCAT 或任意 teardown 顺序验证。下文 Round 21 及更早的
> legacy facade/native-runner 状态仅保留历史追踪；与本补充冲突时以当前工作区
> 源码、view 契约和本补充为准。
>
> Engine 生命周期约束：每次 `build_server_manager_from_db()` 创建一个共享 Engine，
> CLI `-a` 在同一次 build 内枚举 connection；成功、loader 异常及未注册协议路径
> 均 dispose exactly once。manager 持有已物化配置帧及 workers，协议 runtime 不再
> 访问 DB。各 loader 仍使用独立短只读事务，未实现跨 loader single REPEATABLE
> READ snapshot。直接 `db_url` 构造 loader 的内部 Engine 由实例持有，不属于
> composition root 的确定性释放保证。上述 ownership 契约由 12 个定向单元测试
> 验证，不等同数据库并发一致性、连接池压力或全仓回归验证。
>
> Protocol identifier 数据库责任契约：`vw_comm_connection.protocol` 直接来自
> `meta_comm_protocol.protocol_identifier` 并原样作为 registry key，Starfish 不做
> normalization，也不重复检查 NULL、空值、类型或格式。数据库 schema 与主数据保证
> 字段有效；composition 负责 registry 未注册与配置帧协议归属矛盾，协议 backend
> 负责拒绝非法 `station_role`。IEC104/ADS loaders 不做协议有效性检查，且把
> `protocol_role` 逐字映射为 `station_role`。loaders 仍不提供 single snapshot。
>
> pandas-first 公共配置契约：协议点位 view 不含 `connection_id` 或 `measurement`，
> DB adapter 在 SQL 中通过 `vw_src_point_table.point_table_id` JOIN 回连
> connection；当前语义字段只有 `business_semantic_identifier` 与
> `business_semantic_name_zh`，不存在
> `business_semantic_name_en`。DB loaders、`ServerDefinitionLoaderPort`、composition、
> manager 配置和 `ServerFactoryPort` 均以 58 列、一行一个 point 的 DataFrame 作为公共
> 配置契约；`allowed_values` 可作为原子离散域 list，单元格不保存 runtime definition
> 或 metadata dict。manager 深拷贝配置、按 connection 分组创建 worker，并用
> pandas 聚合 `describe()`。只有 `Iec104Server` / `AdsServer` 把单 connection DataFrame
> 转成 runtime dataclass，backend/native 不依赖 pandas。connection 过滤和顺序已下推
> SQL；`connection_ids is None` 不生成 `WHERE`，指定 IDs 使用 expanding bind `IN`，
> 空列表在查询前拒绝，查询后只保留请求 ID 缺失校验，不做 pandas 二次过滤/排序、
> 角色/协议归一化、协议有效性或 Point 覆盖校验。当前真实配置为 IEC104 2 个
> connection/6 points、ADS 1 个 connection/4 points；unit + architecture 52 passed，
> 真实 native integration 6 passed、0 skipped；数据库只读证据时间为
> `2026-08-10T02:03:26.106069Z`，静态门禁全部通过。未运行全仓、长稳、性能或
> 故障注入。
>
> IEC104 当前无独立任务模型：`TaskDefinition`、`ServerDefinition.tasks`、
> `execute_task()`、`task_count`、合成 task ID 与 backend task
> dispatcher/retry/deadline 链均已删除。总召、单点读和累计量召唤由真实外部
> `c104.Client` 经协议 handler 发起；周期、自发、背景传输和命令接收直接由 Point
> metadata/registered points 驱动。CLI 不输出 task 信息。

> Starfish -- 多协议 server simulator 工具层。承接 Seahorse 导出的 ServerPlan JSON 契约，提供协议 server 模拟运行时，供平台调试、联调和数字孪生前置验证使用。
> 最后更新: 2026-06-07 (Round 21: **Starfish 能力增强阶段总收口** — 第 21 轮文档定版与剩余项清理。**本轮核心动作**：修复 test-validator 识别的 6 处文档与代码不一致（见 §6 一致性扫描）+ 建立最终协议能力矩阵（12 协议 final state）+ 清理已完成但仍 deferred 的旧项（Seahorse flaky、LinkLayer timer skeleton、balanced FCB auto flip、retry ERROR、ShortFloat duck typing、Modbus facade register_encoding 接入、IEC101 C_SE_TA_1/C_SE_TB_1/C_SE_TC_1 均已收口于 Round 19/20）+ 保留真实剩余项（6 类：真实 IEC101 server / 真实串口 / 完整 balanced/unbalanced runtime / GOOSE/SV L2 / Beckhoff_ADS 真实环境 / Modbus 真实设备 / 现场部署）。**测试统计定版**：**1416 stable passed（1215 starfish + 15 architecture + 186 seahorse = 180 stable + 5 新 daily_power 稳定性测试 + 1 原 daily_power_preset）** / **0 failed** / **0 flaky**（test-validator 独立验证连续 12 次 0 flaky；`test_curve_daily_power_preset` **根因已修复**——`src/seahorse/strategies/curve_generation.py` 在 noise 叠加后强制 `min(values) >= floor_ratio * baseline = 0.2 * 1500.0 = 300.0`，连续 10 次 0 flaky，**不**再列入 pre-existing flaky）；**协议能力矩阵 12 协议最终态**（按实现形态分类）：HTTP_REST -> real；MODBUS_TCP -> real + register_encoding typed helper；MODBUS_RTU -> rtu-lightweight + register_encoding typed helper；MQTT -> lightweight real + subscribe；OPC_UA -> native runner real-mode / env-dependent；IEC104 -> native runner real-mode / env-dependent；IEC61850_MMS -> native runner real-mode / env-dependent；IEC61850_Report -> native runner real-mode + ReportQueue / env-dependent；IEC101 -> codec-enhanced-plus + LinkLayer runtime skeleton，非 server；Beckhoff_ADS -> codebase-pending / dependency probe；GOOSE -> environment-pending；SV -> environment-pending。**显式边界声明**（**不得**改写为 true）：`supports_server=false` / `supports_serial_runtime=false` / `supports_write_runtime=false` / `register_encoding_runtime=false`。**已清理 legacy 清单**（Round 19/20 已收口，本轮仅做文档同步）：Seahorse `test_curve_daily_power_preset` flaky（**根因已修复**，floor 钳制 floor_ratio=0.2）；IEC101 LinkLayer timer skeleton（**已实现**——LinkLayerTimerService 抽象 + Default threading.Timer + Fake + 完整 send/receive/on_timeout 状态机）；balanced FCB auto flip（**已实现**——ACK+FCV=1+mode=BALANCED 自动翻；NACK/timeout/FCV disabled/unbalanced 不翻）；retry ERROR（**已实现**——retry_count > max_retries 显式进入 ERROR）；ShortFloat duck typing（**已实现**——int / float / `numbers.Real` / `__float__` 统一入口，**不引入 numpy 硬依赖**）；Modbus facade register_encoding 接入（**已实现**——encode_register_value / decode_register_value / register_encoding_capabilities 三个公共方法真实调用工具）；IEC101 C_SE_TA_1 / C_SE_TB_1 / C_SE_TC_1（**已实现**于 Round 19）。**真实剩余项**（**不**得高估为已实现）：真实 IEC101 server / 真实串口通信 / 完整 balanced/unbalanced runtime / GOOSE/SV L2 环境 / Beckhoff_ADS 真实环境 / Modbus 真实设备验证 / 现场部署。本仓库项目名为 BlueCrystal，**BlueOcean_REQ_*.md 在仓库中不存在**，本轮沿用 BlueCrystal_REQ_*.md 体系（已通过 git mv 从原 Whale_REQ_*.md 改名，保留 git 历史），**不新建 BlueOcean_REQ_*.md**；本轮未新增独立架构文档项，因为本次仅做文档/状态定版，未引入新的接口契约、schema 变化或架构边界变化；20→21 轮总收口完成)
...
dead path / dead fixture 清理：Round 12 完成 runner_resolution.py dev fallback 路径迁移（tools/source_lab -> src/starfish/infrastructure/native/bin）、conftest.py 死 fixture 删除、scripts/whale_test.sh source_lab 残留清零、全仓 dead import 路径清零（零 import tools.xxx）；12 轮递进建设全部完成
...
| Round 11 | SourceLab / Tools 物理删除与 Legacy Purge 总收口 | tools/source_lab/ 整目录物理删除（57755 行）、3 个 source_lab shell 脚本删除、tests/support/source_lab_runtime.py 删除、3 个 source_lab 集成测试删除、5 个 Starfish facade C binary 路径迁移至 src/starfish/infrastructure/native/bin/、全仓零 import tools.source_lab、621 passed + 27 skipped |
| Round 12 | Dead Path / Dead Fixture / 过时入口最终清理 | runner_resolution.py `_dev_fallback_candidate()` 和 `is_source_lab_dev_runner_path()` 内部路径改为 `src/starfish/infrastructure/native/bin`；conftest.py 6 个死 fixture（opcua_sim_fleet、opcua_server_runtime 等）删除；scripts/whale_test.sh 无 source_lab 组件残留；全仓 `import tools.xxx` 零结果；全仓零 dead import 路径；629 passed（394 starfish + 181 seahorse + 15 architecture + 31 shared persistence + 6 runner_resolution + 2 ingest boundary）+ 27 skipped（环境 pending，非本轮引入）；validate_shared_source_production_runner.sh ALL VALIDATIONS PASSED；import boundary starfish/seahorse/whale 三方清洁；12 轮递进建设全部完成 |
...
`whale.ingest.diagnostics/probe.py` 提供 `probe_facade(facade, plan=None, read_point_ids=None, skip_start=False)` 对统一 driver 执行最简可用性探测。已覆盖 stub/HTTP_REST/MODBUS_TCP/MQTT/OPC_UA/IEC104 等 runtime driver。

探测序列：
1. 如提供 plan，先执行 `facade.load_points(plan)`。
2. 若未跳过启动且 facade 未运行，执行 `facade.start()`。
3. 执行 `facade.health()` 健康检查。
4. 读取一个或多个点位 `facade.read(read_point_ids)`。
5. 返回 `ProbeResult`（status="PASS"|"FAIL"|"NOT_RUN"、reason 说明）。

`ProbeResult` dataclass 字段：`endpoint_id`、`protocol`、`mode`、`status`、`reason`、`details`。

FAIL 条件：`load_points` / `start` / `health` / `read` 任一步骤异常。unavailable 等 runtime 不可用语义由返回 reason 与 mode 表达，不高估为 PASS。
...
安全边界：该能力位于 ingest 侧诊断包，不反向 import `starfish`；不做生产级诊断。

### SF-FR-013 profile 最小工具能力

`whale.ingest.diagnostics/profile.py` 提供 `profile_facade(facade, iterations=10, point_ids=None)`，对统一 driver 执行 `iterations` 次 `facade.read(point_ids)` 操作并统计耗时。

返回 `ProfileResult` dataclass 字段：`endpoint_id`、`protocol`、`mode`、`iterations`、`duration_ms`、`stats`、`samples`、`status`、`reason`。

`iterations <= 0` 时返回 `ProfileResult`，状态为 `FAIL`。
...
安全边界：该能力位于 ingest 侧诊断包，不反向 import `starfish`。

### SF-FR-014 capacity 最小工具能力

`whale.ingest.diagnostics/capacity.py` 提供 `capacity_scan(facade, point_ids=None, read_count=10)`，对统一 driver 执行轻量端点/点位/读取容量扫描。

返回 `CapacityResult` dataclass 字段：`endpoint_id`、`protocol`、`mode`、`status`、`point_count`、`max_tested_points`、`read_count`、`reason`。

NOT_RUN 条件：unsupported protocol 或 facade mode="unavailable"。FAIL 条件：health 失败或 read 连续异常。
...
- **protocol codec（Round 14 头部 + Round 15 增强）**：`src/starfish/domain/protocols/iec101/` 提供：
...
Starfish 提供 `src/starfish/domain/protocols/iec101/` 包，包含 IEC 101 协议帧字段编解码骨架（纯 Python，zero external dependency）。这不是完整的 IEC101 server（无串口通信层、无链路层控制字段、无平衡/非平衡传输模式、无 IEC 60870-5-101 状态机）。
...
- `src/starfish/domain/protocols/iec101/time.py` / `link_layer.py` / `information_object.py` / `information_elements.py` 均为纯 Python 实现，零外部依赖。
...
Starfish 提供统一的 native runner 管理框架（`src/starfish/infrastructure/native/`），包含 runner 规格定义、统一 binary 探查和子进程生命周期管理三个模块，供 OPC_UA/IEC104/IEC61850 等依赖 C runner 子进程的 protocol facade 使用。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| SF-FR-001 | 总体逻辑设计 8.2/8.3 | ServerPlan JSON loader（9 项校验 + payload_hash 复算） | FR | 高 | starfish.adapters.drivers | P1+P2 | 已完成 | `src/starfish/adapters/config/server_config_loader.py`；load_server_plan 支持文件加载、9 项校验（必填字段/schema_version/scenario_id/synthetic/endpoints/points/capabilities/initial_values/payload_hash 复算）、LoadResult/ValidationResult 结构化返回；边界错误 FileNotFoundError/JSONDecodeError/ValueError | `tests/unit/starfish/test_server_plan_loader.py` -> ~30 tests passed | 无立即差距；校验规则后续可按需扩展 | 持续维护 | 2026-06-11 |
| SF-FR-002 | 总体逻辑设计 8.2/8.3 | ServerSimulatorFacade 最小运行时生命周期（start/stop/health/read/load_points/update_values/capabilities） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/simulator/server_simulator_facade.py`；in-memory stub 实现；7 个已实现方法：start/stop/health/load_points/read/update_values/capabilities；幂等安全 | `tests/unit/starfish/test_server_simulator_facade.py` -> ~30 tests passed | write/subscribe/report 为 NOT_IMPLEMENTED；真实协议 server 未启动 | 持续按协议补足 real mode | 2026-06-11 |
| SF-FR-003 | 总体逻辑设计 8.2/8.3 | read initial_values（load_points 从 plan.initial_values 填充） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `ServerSimulatorFacade.load_points()` 复制 plan.initial_values；`read()` 返回存储值；覆盖/空 initial_values/reload 场景通过 | `tests/unit/starfish/test_server_simulator_facade.py` -> read/load_points/initial_values 测试通过 | 无 | 无 | 2026-06-11 |
| SF-FR-004 | 总体逻辑设计 8.2 | unsupported write/subscribe/report 返回 NOT_IMPLEMENTED（UnsupportedOperation 异常） | FR | 高 | starfish.adapters.drivers, starfish.domain | P1 | 已完成 | `src/starfish/domain/server_config.py` 中定义 `UnsupportedOperation`；driver 的 write/subscribe/report 明确抛出 NOT_IMPLEMENTED 中文错误；CLI 单测覆盖错误路径 | `tests/unit/starfish/test_server_simulator_facade.py`；`tests/unit/starfish/test_starfish_cli.py` | 不得伪装实现完成 | 待后续真实实现逐项替换 | 2026-06-11 |
| SF-FR-005 | 总体逻辑设计 8.2/8.3 | CLI 统一运行入口（validate-plan / describe / health / read / run） | FR | 高 | starfish.__main__ | P1 | 已完成 | `src/starfish/__main__.py`；CLI 统一经 `starfish.api` 打开 runtime，提供 `validate-plan`、`describe`、`health`、`read`、`run` 五个子命令；文件不存在/无效 JSON 返回非零 | `tests/unit/starfish/test_starfish_cli.py`；`tests/unit/starfish/test_runtime_api.py` | 无 | 后续按需增加命令，但保持统一高层入口 | 2026-06-11 |
| SF-FR-006 | 总体逻辑设计 8.2/8.3 | HTTP_REST 真实 server 生命周期（ThreadingHTTPServer, GET /points） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/http/http_rest_facade.py`；HttpRestFacade 使用 http.server.ThreadingHTTPServer 启动真实 HTTP 服务端；GET /points 返回 JSON；start/stop/health/load_points/read/update_values/capabilities 实现；write/subscribe/report 为 UnsupportedOperation | `tests/unit/starfish/test_protocol_facade.py` -> HttpRestFacade 测试通过（start/stop/health/read/initial_values/update_values/capabilities/NOT_IMPLEMENTED/real HTTP GET） | HTTP_REST write 为 NOT_IMPLEMENTED；subscribe/report 为 NOT_IMPLEMENTED | 后续按需实现 write 语义 | 2026-06-11 |
| SF-FR-007 | 总体逻辑设计 8.2/8.3 | MODBUS_TCP 真实 server 生命周期（TCP socket, FC03/FC06） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/modbus/modbus_tcp_facade.py`；ModbusTcpFacade 使用 Python 标准库 socket 启动真实 TCP server；支持 FC03 Read Holding Registers（真实 Modbus 帧编码）和 FC06 Write Single Register（写入后 FC03 回读验证）；start/stop/health/load_points/read/write/update_values/capabilities 实现；subscribe/report 为 UnsupportedOperation | `tests/unit/starfish/test_protocol_facade.py` -> ModbusTcpFacade 测试通过（FC03/FC06 真实读写/start/stop/health/read/write/update_values/capabilities/NOT_IMPLEMENTED） | 未实现异常码完整矩阵/浮点寄存器/多单元 ID；subscribe/report 为 NOT_IMPLEMENTED | 后续按需扩展 Modbus 功能码和异常码 | 2026-06-11 |
| SF-FR-008 | 总体逻辑设计 8.2/8.3 | RuntimeRegistry 协议 dispatch + native runner dispatch（real/stub/unavailable 三模式） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/factory.py`；`create_driver_for_endpoint` 根据 protocol 分发：HTTP_REST->HttpRestFacade（mode="real"）、MODBUS_TCP->ModbusTcpFacade（mode="real"）、MQTT->MqttFacade（mode="mqtt-lightweight"）、OPC_UA->OpcUaFacade（mode="real"|"unavailable"）、IEC104->Iec104Facade（mode="real"|"unavailable"）、IEC61850_MMS->Iec61850MmsFacade（mode="real"|"unavailable"）、IEC61850_Report->Iec61850ReportFacade（mode="real"|"unavailable"）、其他->ServerSimulatorFacade（mode="stub"）；`get_supported_protocols()` 返回主协议清单 | `tests/unit/starfish/test_protocol_facade.py`；`tests/unit/starfish/test_opcua_iec104_facade.py`；`tests/unit/starfish/test_iec61850_facade.py` | 其他协议真实 server 启动为 codebase-pending 或 environment-pending | 后续按协议增加 real mode factory | 2026-06-11 |
| SF-FR-009 | 总体逻辑设计 8.2/8.3 | CLI 按 endpoint 输出运行模式与健康信息 | FR | 高 | starfish.__main__ | P1 | 已完成 | `src/starfish/__main__.py` 的 `describe` / `health` 子命令按 endpoint 输出 protocol、mode、available、reason 与 health 信息；不再保留旧 smoke-server-plan 入口 | `tests/unit/starfish/test_starfish_cli.py` | 无 | 持续维护 | 2026-06-11 |
| SF-FR-010 | 总体逻辑设计 8.2/8.3 | MQTT lightweight JSON-line TCP facade（不是完整 MQTT broker） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/mqtt/mqtt_facade.py`；MqttFacade 使用 Python 标准库 socket bind/listen/accept daemon 线程 TCP server；JSON-line 通信；SubscriptionQueue 封装 queue.Queue；start/stop/health/load_points/read/update_values/capabilities/subscribe 实现；write/report 为 UnsupportedOperation | `tests/unit/starfish/test_mqtt_facade.py` -> 37 tests passed（SubscriptionQueue/lifecycle/data/subscribe/NOT_IMPLEMENTED/TCP protocol/smoke flow） | MQTT 不等同于完整 MQTT broker（无 CONNECT/CONNACK/QoS/topic tree/retained/session）；write 为 NOT_IMPLEMENTED；report 为 NOT_IMPLEMENTED | 后续按需扩展为标准 MQTT v3.1.1 broker | 2026-06-11 |
| SF-FR-011 | 总体逻辑设计 8.2/8.3 | subscribe 语义 — SubscriptionQueue（queue.Queue 封装） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `MqttFacade.subscribe` 返回 `SubscriptionQueue`（queue.Queue 封装）；update_values 后通知所有匹配 subscriber；TCP publish 触发 subscribe 通知；多个独立 subscriber 各自接收；load_points 清空旧订阅；get/get_nowait 阻塞/非阻塞取 | `tests/unit/starfish/test_mqtt_facade.py` -> TestMqttFacadeSubscribe 7 tests passed（subscribe queue/update notify/multiple points/unrelated filter/multiple subscribers/clear on reload/TCP publish sync） | 当前仅 MqttFacade 实现 subscribe；HttpRestFacade/ModbusTcpFacade/ServerSimulatorFacade subscribe 仍为 UnsupportedOperation | 后续扩展其他 drivers 的 subscribe 语义 | 2026-06-11 |
| SF-FR-012 | 总体逻辑设计 8.2 | probe 最小诊断能力（启动-健康-读取探测，已扩展至 OPC_UA/IEC104/IEC61850） | FR | 高 | whale.ingest.diagnostics | P1 | 已完成 | `src/whale/ingest/diagnostics/probe.py`；`probe_facade` 对统一 driver 执行 start/health/read 探测序列；`ProbeResult` dataclass；支持 `skip_start`；OPC_UA/IEC104/IEC61850 real mode 时 PASS，unavailable 时 NOT_RUN + reason | `tests/unit/starfish/test_probe_profile_capacity.py` -> probe 部分 ~15 tests passed（stub/http_rest/modbus/mqtt/opcua/iec104/iec61850_mms/iec61850_report/specific_points/skip_start） | 不等同生产级诊断工具 | 持续维护 | 2026-06-11 |
| SF-FR-013 | 总体逻辑设计 8.2 | profile 最小诊断能力（read N 次采样耗时统计，已扩展至 OPC_UA/IEC104/IEC61850） | FR | 高 | whale.ingest.diagnostics | P1 | 已完成 | `src/whale/ingest/diagnostics/profile.py`；`profile_facade` 对统一 driver 执行 iterations 次 read 统计耗时；OPC_UA/IEC104/IEC61850 real mode 时 PASS，unavailable 时 NOT_RUN + reason | `tests/unit/starfish/test_probe_profile_capacity.py` -> profile 部分 ~10 tests passed（stub/http_rest/mqtt/opcua/iec104/iec61850_mms/iec61850_report/zero_iterations） | 不等同生产级性能测试 | 持续维护 | 2026-06-11 |
| SF-FR-014 | 总体逻辑设计 8.2 | capacity 最小诊断能力（端点/点位/读取容量扫描，已扩展至 OPC_UA/IEC104/IEC61850） | FR | 高 | whale.ingest.diagnostics | P1 | 已完成 | `src/whale/ingest/diagnostics/capacity.py`；`capacity_scan` 对统一 driver 执行轻量容量扫描；支持 HTTP_REST/MODBUS_TCP/MQTT/OPC_UA/IEC104/IEC61850_MMS/IEC61850_Report | `tests/unit/starfish/test_probe_profile_capacity.py` -> capacity 部分 ~11 tests passed（http_rest/modbus/mqtt/opcua/iec104/iec61850_mms/iec61850_report/unsupported_protocol/zero_points） | 不等同生产容量验收或压测 | 持续维护 | 2026-06-11 |
| SF-FR-015 | 总体逻辑设计 8.2/8.3 | OPC_UA real mode — open62541 C runner 子进程（dependency probe + start/stop/health/read） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/opcua/opcua_facade.py`；OpcUaFacade 使用 open62541 C runner 子进程；dependency probe 检测二进制存在性；mode="real"|"unavailable"；start->subprocess.Popen->READY signal->TCP connect；幂等安全；write/subscribe/report 为 UnsupportedOperation | `tests/unit/starfish/test_opcua_iec104_facade.py`；`tests/unit/starfish/test_protocol_facade.py` | write/subscribe/report NOT_IMPLEMENTED；不等同完整 OPC UA server | 后续按需扩展 read 真实 OPC UA 数据交换 | 2026-06-11 |
| SF-FR-016 | 当前 Whale view-driven Starfish | IEC104 Source simulator：从 comm/src views 装配 endpoint、Point 与协议能力 | FR | 高 | `starfish.adapters.db_views.iec104`、`starfish.adapters.protocols.iec104` | P1 | 已完成（定向验证） | `Iec104DbViewLoader` 只读 `vw_comm_iec104_connection`、`vw_src_point_table`、`vw_src_iec104_point_item`；Point view 能力字段直接映射为 metadata/capabilities；总召、读、累计量召唤走 c104 handler，周期、自发、背景和命令由 registered points/metadata 驱动 | `tests/unit/starfish/test_iec104_db_view_loader.py`、`test_iec104_server.py`、`test_iec104_backend.py`；真实外部 c104.Client 完成连接、总召、单点读、自发上送；最新定向 unit + architecture 52 passed、真实 native integration 6 passed/0 skipped | 未跑全仓回归、长稳、性能或网络故障注入；不支持组召、组累计量召唤、累计量冻结/复位与 CP24Time2a Point | 持续按 view/Point 契约扩展，并保持标准协议 client 入口与 GPLv3 分发审查边界 | 2026-08-10 |
| SF-FR-017 | 总体逻辑设计 8.2 | dependency probe 与 unavailable 语义（二进制探测 + 文件大小 + 可执行权限） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `OpcUaFacade.probe_opcua_binary()`、`Iec104Facade.probe_iec104_binary()`、`Iec61850MmsFacade.probe_iec61850_binary()`、`Iec61850ReportFacade.probe_iec61850_binary()` 使用 os.stat + os.access 检测；mode="unavailable" 时 start/stop 不执行子进程；probe/profile/capacity 返回 NOT_RUN + reason | `tests/unit/starfish/test_opcua_iec104_facade.py`；`tests/unit/starfish/test_iec61850_facade.py`；`tests/unit/starfish/test_probe_profile_capacity.py` | 不得把 unavailable 写为 PASS | 持续维护依赖探测逻辑 | 2026-06-11 |
| SF-FR-018 | 总体逻辑设计 8.2/8.3 | IEC61850 MMS real mode -- iec61850_simulator_server C runner 子进程（dependency probe + start/stop/health/read 内存点位） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/iec/iec61850_mms_facade.py`；Iec61850MmsFacade 使用 iec61850_simulator_server C runner 子进程；dependency probe 检测二进制存在性；mode="real"|"unavailable"；start->subprocess.Popen->READY signal->TCP connect；幂等安全；read 为内存点位（非真实 MMS 协议帧）；write/subscribe 为 UnsupportedOperation | `tests/unit/starfish/test_iec61850_facade.py`；`tests/unit/starfish/test_protocol_facade.py`；`tests/unit/starfish/test_probe_profile_capacity.py` | MMS read 为内存点位非真实协议帧；write/subscribe NOT_IMPLEMENTED；不等同完整 IEC 61850 MMS server | 后续按需实现真实 MMS 协议帧 read | 2026-06-11 |
| SF-FR-019 | 总体逻辑设计 8.2/8.3 | IEC61850 Report real mode + ReportQueue event 语义（iee61850_report_runner C runner 子进程 + ReportQueue put/get/drain/FIFO） | FR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/iec/iec61850_report_facade.py`；Iec61850ReportFacade 使用 iec61850_report_runner C runner 子进程；dependency probe 检测二进制存在性；mode="real"|"unavailable"；start->subprocess.Popen->READY signal；ReportQueue 封装 queue.Queue（put/get/get_nowait/drain/FIFO）；events 来自 Python 侧（非子进程真实 report 数据）；write/subscribe 为 UnsupportedOperation | `tests/unit/starfish/test_iec61850_facade.py`；`tests/unit/starfish/test_protocol_facade.py`；`tests/unit/starfish/test_probe_profile_capacity.py` | report runner 子进程在 real mode 测试中未实际启动（events 来自 Python 侧）；write/subscribe NOT_IMPLEMENTED；不等同完整 IEC 61850 Report server | 后续按需实现子进程真实 report 数据流 | 2026-06-11 |
| SF-FR-020 | 总体逻辑设计 8.2 | IEC101 facade 与 codec-enhanced mode（含 SIQ/QDS/NVA/M_SP_NA_1/M_DP_NA_1/M_ME_NA_1/C_SC_NA_1/ASDU 列表 SQ=0/SQ=1/FT1.2 帧/codec_capabilities 显式声明） | FR | 中 | starfish.adapters.drivers | P1 | 已完成（Round 15 升级为 codec-enhanced） | `src/starfish/adapters/drivers/iec/iec101_facade.py`；Iec101Facade in-memory stub；mode 动态判定（codec-enhanced/codec-skeleton/environment-pending/codebase-pending 4 级）；health() 含增强诊断（pty_available/lib60870_exists/iec101 binaries/codec_skeleton_ready/codec_enhanced_ready）；codec_capabilities() 返回 [codec_mode, supported_type_ids, supports_ft12_frame_codec, supports_server=false, supports_serial_runtime=false]；probe_iec101_codec_enhanced() 验证信息体/对象/列表/帧/校验和全部可用 | `tests/unit/starfish/test_remaining_protocols.py`；`tests/unit/starfish/test_iec101_codec.py`；`tests/unit/starfish/test_iec101_information_elements.py`；`tests/unit/starfish/test_iec101_asdu_objects.py`；`tests/unit/starfish/test_iec101_ft12_frame.py` | 无串口通信层、无 IEC 60870-5-101 状态机、无平衡/非平衡模式、CP56Time2a 时标 IE deferred、QU 字段语义简化 | 待后续实现串口通信层+状态机+CP56Time2a IE 后升级为 real mode | 2026-06-11 |
| SF-FR-021 | 总体逻辑设计 8.2 | MODBUS_RTU rtu-lightweight PTY-backed real mode（含 CRC16/8 FCs/4 异常码） | FR | 高 | starfish.adapters.drivers | P1 | 已完成（Round 14 升级） | `src/starfish/adapters/drivers/modbus/modbus_rtu_facade.py`；ModbusRtuFacade 使用 pty.openpty() PTY pair 实现两模式 dispatch（PTY 可用→rtu-lightweight，不可用→codebase-pending）；CRC16 标准算法（0xA001 多项式）；8 个功能码覆盖全部 4 个数据区；4 个异常码（0x01-0x04）支持；PTY 生命周期（openpty/start/stop/health）；write(read) 线程安全 | `tests/unit/starfish/test_modbus_rtu_facade.py` | PTY 不等同真实串口（无 RS-232/RS-485 电气特性、波特率、奇偶校验、停止位）；帧边界通过 CRC 逐字节探测模拟（非真实 3.5 字符间隔）；未实现浮点/32-bit 寄存器编解码、多从站 ID、广播地址支持 | 真实串口现场验证需 RS-232/RS-485 硬件环境 | 2026-06-11 |
| SF-FR-022 | 当前 Whale view-driven Starfish | ADS AMS/TCP Source simulator：从 comm/src views 装配 endpoint、symbol/index 点位与 notification 约束 | FR | 中 | `starfish.adapters.db_views.ads`、`starfish.adapters.protocols.ads` | P1 | 已完成（定向验证） | `AdsDbViewLoader` 只读 `vw_comm_ads_connection`、`vw_src_point_table`、`vw_src_ads_point_item` 并映射 `AdsServer` / `AdsTcpBackend`；支持 `READ_STATE`、symbol handle 读取、INDEX 读取、CYCLIC/ON_CHANGE notification；Source 普通写拒绝 | `tests/unit/starfish/test_ads_db_view_loader.py`；`tests/integration/starfish/test_view_protocol_servers.py`；第二轮 validator 的当前 DB client 验收覆盖 3 个 CYCLIC、1 个 ON_CHANGE、4 次 DELETE、删除后无回调，并在支持关闭顺序后确认 client/notification 计数为 0 | 未跑全仓回归、长稳、性能、故障注入或现场 TwinCAT；活动 callback 未 DELETE 时任意 teardown 可能触发 AdsLib SIGSEGV | 固定支持顺序：DELETE notification -> client close -> session cleanup -> server stop；后续补充 native teardown 故障隔离 | 2026-08-08 |
| SF-FR-023 | 总体逻辑设计 8.2 | GOOSE facade 与 environment-pending 定版 | FR | 中 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/iec/goose_facade.py`；GooseFacade 继承 ServerSimulatorFacade in-memory stub；mode="environment-pending"；lifecycle 继承 in-memory stub；write/subscribe/report 为 NOT_IMPLEMENTED；`probe_goose_binary()` 探测函数保留 | `tests/unit/starfish/test_remaining_protocols.py`；`tests/unit/starfish/test_iec61850_facade.py` | L2 veth 网络环境未就绪（需 raw socket CAP_NET_RAW）；ieee61850_goose_publisher_simulator.c 已存在于 third_party/ | 待 L2 veth 环境就绪后升级为 real mode | 2026-06-11 |
| SF-FR-024 | 总体逻辑设计 8.2 | SV facade 与 environment-pending 定版 | FR | 中 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/iec/sv_facade.py`；SvFacade 继承 ServerSimulatorFacade in-memory stub；mode="environment-pending"；lifecycle 继承 in-memory stub；write/subscribe/report 为 NOT_IMPLEMENTED；`probe_sv_binary()` 探测函数保留 | `tests/unit/starfish/test_remaining_protocols.py` | L2 veth + PTP 时间同步环境未就绪（需 raw socket CAP_NET_RAW + IEEE 1588）；ieee61850_sv_publisher_simulator.c 已存在于 third_party/ | 待 L2 veth + PTP 环境就绪后升级为 real mode | 2026-06-11 |
| SF-FR-025 | 总体逻辑设计 8.2 | native runner 管理框架（NativeRunnerSpec + probe_native_runner + NativeProcessHandle） | FR | 高 | starfish.infrastructure.native | P1 | 已完成 | `src/starfish/infrastructure/native/runner_spec.py` — NativeRunnerSpec dataclass；`src/starfish/infrastructure/native/runner_probe.py` — probe_native_runner() 统一 binary 探测；`src/starfish/infrastructure/native/process_handle.py` — NativeProcessHandle 子进程生命周期管理（start/stop/health/pid/pid_file cleanup）；所有函数中英文 docstring 完整 | `tests/unit/starfish/test_native_runner_framework.py` -> 66 tests passed（NativeRunnerSpec 构造/字段/序列化；probe_native_runner 二元/缺失/太小/不可执行/unavailable reason；NativeProcessHandle start/stop/health/pid/pid_file/cleanup/lifecycle edge cases/subprocess integration） | 各 facade 尚未内部迁移至此框架（仍使用各自 subprocess.Popen 逻辑） | 后续逐协议迁移 facade 子进程管理层至 native runner 框架 | 2026-06-05 |
| SF-FR-026 | 总体逻辑设计 8.2 | MODBUS_RTU rtu-lightweight PTY-backed real mode（CRC16/8 FCs/4 异常码） | FR | 高 | starfish.adapters.drivers | P1 | 已完成（Round 14 升级） | `src/starfish/adapters/drivers/modbus/modbus_rtu_facade.py`；CRC16 标准算法（0xA001 多项式 5 个已知向量）；8 个功能码覆盖全部 4 个数据区；4 个异常码（0x01-0x04）；pty.openpty() PTY 生命周期（openpty/start/stop/health）；两模式 dispatch（PTY 可用→rtu-lightweight，不可用→codebase-pending） | `tests/unit/starfish/test_modbus_rtu_facade.py` -> 76 tests passed（CRC16 5 向量/8 FCs 帧编解码/异常码 0x01-0x04/PTY 生命周期/两模式 dispatch/codebase-pending fallback/NOT_IMPLEMENTED subscribe/report） | PTY 不等同真实串口（无 RS-232/RS-485 电气特性/波特率/奇偶校验/停止位）；帧边界 CRC 逐字节探测（非 3.5 字符间隔）；未实现浮点/32-bit 寄存器编解码/多从站 ID/广播地址支持 | 真实串口现场验证需 RS-232/RS-485 硬件 | 2026-06-11 |
| SF-FR-027 | 总体逻辑设计 8.2 | IEC101 codec-skeleton -- TypeId/COT/ASDUHeader/IOA/CA 编解码骨架 | FR | 高 | starfish.domain.protocols.iec101 | P1 | 已完成（Round 14 新增，Round 15 文档统一为实测 26 values） | `src/starfish/domain/protocols/iec101/` 含 types.py(TypeId 26/COT 26 枚举，实测)、asdu.py(ASDUHeader 6 bytes encode/decode)、ioa.py(IOA 3 bytes encode/decode)、common_address.py(CA 2 bytes encode/decode)；纯 Python 实现，zero external dependency | `tests/unit/starfish/test_iec101_codec.py` -> 40 tests passed（TypeId/COT 枚举值验证 + ASDUHeader/IOA/CA encode/decode + 边界） | 无串口通信层、无链路层控制字段、无 IEC 60870-5-101 状态机、无完整 server 生命周期；非完整 IEC101 server | 待后续实现完整串口通信层和状态机后可与 facade 集成为完整 IEC101 server | 2026-06-06 |
| SF-FR-028 | 总体逻辑设计 8.2 | IEC101 codec-enhanced -- 信息体 + ASDU 列表 SQ=0/SQ=1 + FT1.2 帧 + checksum | FR | 高 | starfish.domain.protocols.iec101, starfish.adapters.drivers | P1 | 已完成（Round 15 新增） | `src/starfish/domain/protocols/iec101/quality.py` SIQ/QDS 质量描述符 + `information_elements.py` NVA 归一化值（16-bit signed，[-1.0, +1.0-1/32768]）+ `information_object.py` M_SP_NA_1/M_DP_NA_1/M_ME_NA_1/C_SC_NA_1 信息对象 + `codec.py` ASDU 信息对象列表编解码（SQ=0 独立地址/SQ=1 顺序地址 + UnknownAsduError 安全解码）+ `frame.py` FT1.2 固定/可变帧 + checksum + 长度不一致检测；`src/starfish/adapters/drivers/iec/iec101_facade.py` 提供 codec capabilities 与 runtime mode 封装 | `tests/unit/starfish/test_iec101_information_elements.py`；`tests/unit/starfish/test_iec101_asdu_objects.py`；`tests/unit/starfish/test_iec101_ft12_frame.py` | 4 TypeId 限制（M_SP_NA_1/M_DP_NA_1/M_ME_NA_1/C_SC_NA_1）；QU 字段语义简化；CP56Time2a 7 字节时标 IE 显式 deferred；非完整 server（supports_server=false/supports_serial_runtime=false）；不平衡/非平衡状态机未实现 | 待 Round 16 补齐 CP56Time2a IE + 带时标 TypeId + 链路层最小状态机 | 2026-06-11 |
| SF-FR-029 | 总体逻辑设计 8.2 | IEC101 codec-enhanced-plus -- CP56Time2a 7 字节时标 IE + **17 TypeID 矩阵（Round 19 扩展 14 → 17：4 不带时标监视 + 1 不带时标标度化 + 1 不带时标短浮点 + 4 不带时标命令 + 3 带时标命令 C_SE_TA_1/C_SE_TB_1/C_SE_TC_1（Round 19 新增）+ 5 带时标监视 = 17 个；以 capability 实际值 17 为准，严禁硬写 13/14/15/18）** + ShortFloat IEEE 754 32-bit IE + ScaledValue IE（16-bit signed，Round 18 新增）+ QOS 结构化（SetPointQualifier 枚举 + SetPointCommandQualifier dataclass，Round 18 新增）+ C_SC_NA_1 QU 显式化（CommandPulse + SingleCommandQualifier 子字段 select_execute/qualifier/ql_value/persistent/pulse）+ 5 态链路层 skeleton（IDLE/WAIT_ACK/SEND/RECEIVE/ERROR）+ LinkLayerTimers + t1/t2/t3 + FCB/FCV helper + balanced/unbalanced 差异化 + **supports_command_codec=true / supports_scaled_value=true / supports_write_runtime=false（Round 18 新增）** + **supports_time_tagged_command_codec=true / supported_command_type_ids=7 / supported_time_tagged_command_type_ids=3 / supported_time_tagged_type_ids=8（Round 19 新增）** | FR | 高 | starfish.domain.protocols.iec101, starfish.adapters.drivers | P1 | 已完成（Round 16 起步，Round 17 一次性收口，Round 18 扩展 14，Round 19 扩展 17） | `src/starfish/domain/protocols/iec101/` 承载 17 TypeId 矩阵、CP56Time2a、ShortFloat、ScaledValue、QOS 与 LinkLayer skeleton；`src/starfish/adapters/drivers/iec/iec101_facade.py` 承载 runtime mode、health reason_text、capabilities 暴露与 UnsupportedOperation 边界 | `tests/unit/starfish/test_iec101_asdu_objects.py`；`tests/unit/starfish/test_iec101_codec.py`；`tests/unit/starfish/test_iec101_link_layer.py`；`tests/unit/starfish/test_iec101_information_elements.py` | 链路层 skeleton 仅 5 态；不是 IEC101 server / 真实串口 / 完整链路层 / 真实 write runtime；C_SE_T* command codec 不得被高估为真实写能力 | 真实 IEC101 server + Modbus 真实设备验证仍 deferred | 2026-06-11 |
...
| SF-AR-002 | 总体逻辑设计 2.1/8.2/8.3 | Starfish 不 import seahorse/whale.ingest/whale.shared.source | AR | 高 | 全组件 | P2 | 已建立并通过 | `tests/unit/architecture/test_starfish_import_boundary.py` -> import boundary AST 扫描与目录结构检查通过；同时确认 ingest 侧新 `diagnostics/` 包不反向 import `starfish` | import boundary 双向验证通过 | 无立即差距 | 持续监控 | 2026-06-11 |
| SF-AR-003 | 总体逻辑设计 8.2 | RuntimeRegistry 支持 real/stub/mqtt-lightweight/codec-enhanced-plus/codec-enhanced/codec-skeleton/rtu-lightweight/codebase-pending/environment-pending + native runner dispatch（13 协议 9 模式） | AR | 高 | starfish.adapters.drivers | P1 | 已完成 | `src/starfish/adapters/drivers/factory.py`；`create_driver_for_endpoint` / `create_drivers` 根据 protocol 分发各类 driver；`DriverEntry` 暴露 mode、available、reason；协议能力与 native runner dispatch 由 drivers 层统一装配 | `tests/unit/starfish/test_protocol_facade.py`；`tests/unit/starfish/test_modbus_rtu_facade.py`；`tests/unit/starfish/test_iec101_codec.py` | 1 个协议 codebase-pending（Beckhoff_ADS）/ 2 个协议 environment-pending（GOOSE/SV）；IEC101 为 codec-enhanced-plus 非完整 server；MODBUS_RTU rtu-lightweight 不等同真实串口 | 条件就绪后升级 pending 协议为 real mode；真实 IEC101 server + Modbus 真实设备验证仍 deferred | 2026-06-11 |
| SF-AR-004 | 当前 Whale view-driven Starfish | Composition root 的数据库 Engine 所有权与 runtime 隔离 | AR | 高 | `starfish.composition`、DB view adapters、core manager | P1 | 已完成（定向契约验证） | 每次 `build_server_manager_from_db()` 创建并向通用/协议 loaders 注入同一个 Engine；`-a` 同 build 枚举；成功、loader 异常和未注册协议均 dispose exactly once；manager 持有已物化配置帧和 workers，worker 持有 runtime definitions，启动不再访问 DB | `tests/unit/starfish/test_connection_db_view_loader.py`、`test_starfish_cli.py`：Engine ownership + CLI 定向 12 passed；真实 c104/pyads integration 2 passed；CLI 2 IEC104 + 1 ADS endpoint exit 0 | loaders 各自独立短只读事务，未实现 single REPEATABLE READ snapshot；直接 `db_url` 构造 loader 的实例持有 Engine，不在 composition 确定性释放保证内；未跑连接池压力、并发一致性与全仓回归 | 保持 composition 为 CLI/startup 唯一 DB 资源所有者；需要强一致快照时另行设计共享 Connection/transaction port | 2026-08-09 |
| SF-AR-005 | 当前 Whale view-driven Starfish | Protocol identifier 数据库责任与应用直接使用契约 | AR | 高 | `vw_comm_connection`、DB view loaders、composition registry、protocol backends | P1 | 已完成（定向契约验证） | `vw_comm_connection.protocol` 直接来自 `meta_comm_protocol.protocol_identifier`；通用与协议 loaders 均不做 normalization 或协议有效性检查；composition 拒绝未注册 registry key 与配置帧协议归属矛盾；`protocol_role` 逐字映射为 `station_role`，非法值由协议 backend 装配边界拒绝 | unit + architecture 52 passed；真实 native integration 6 passed/0 skipped；静态门禁通过 | 数据库 schema 与主数据承担 NULL、空值、类型和格式有效性；应用不维护格式变体测试；未跑全仓回归；无 single snapshot | 新协议在数据库主数据与 composition registry 使用同一 identifier；协议角色约束留在 backend 边界 | 2026-08-10 |
| SF-AR-006 | 当前 Whale view-driven Starfish | pandas-first 公共配置契约与 runtime 转换边界 | AR | 高 | DB view adapters、`starfish.core.config_frames`、core ports/manager、composition、protocol factory/server adapters | P1 | 已完成（定向验证） | `SERVER_CONFIG_COLUMNS` 固定 58 列、一行一个 point；DB loaders、`ServerDefinitionLoaderPort`、composition、manager 配置、`ServerFactoryPort` 传递 DataFrame；manager 深拷贝、按 connection 分组并以 pandas 聚合 `describe()`；composition 拒绝 connection 缺失/未请求、protocol 不一致/重复归属及 point 重复等基数错误；协议点位在 SQL 中通过 `point_table_id` JOIN 回连 connection，仅使用 `business_semantic_identifier` / `business_semantic_name_zh`；单元格不保存 runtime definition/metadata dict，`allowed_values` 原子 list 除外；仅 `Iec104Server` / `AdsServer` 转换 runtime dataclass，backend/native 无 pandas | pandas 2.3.3 与 stubs 满足声明；当前真实配置 IEC104 2 connections/6 points、ADS 1 connection/4 points，数据库只读证据时间 `2026-08-10T02:03:26.106069Z`；unit + architecture 52 passed；真实 native integration 6 passed、0 skipped；静态门禁通过 | 未跑全仓、长稳、性能、故障注入；无 pandas 性能收益证据；loaders 仍无 single snapshot；当前点数仅代表该时间点数据库配置 | 保持 DataFrame 作为配置主链公共契约；只允许 protocol server adapter 转换到 runtime dataclass，禁止 pandas 进入 backend/native 并发运行结构 | 2026-08-10 |
| SF-AR-007 | 当前 Whale view-driven Starfish | IEC104 Point 驱动协议能力，不建立独立任务模型 | AR | 高 | core definitions/manager、IEC104 server/backend、CLI | P1 | 已完成（定向验证） | 当前源码无 `TaskDefinition`、`ServerDefinition.tasks`、`execute_task()`、`task_count`、合成 task ID 或 backend task dispatcher/retry/deadline 链；总召、读、累计量召唤由 c104 handler 处理；周期、自发、背景与命令直接由 Point metadata/registered points 驱动 | unit + architecture 52 passed；真实 native integration 6 passed、0 skipped；真实外部 c104.Client 覆盖连接、总召、单点读、自发上送；真实 pyads 覆盖读取、CYCLIC/ON_CHANGE notification 与安全 teardown；静态/类型/docstring/diff gates 通过 | 未跑全仓、性能、长稳或网络故障注入；源码连接等待仍有普通 timeout deadline，不属于已删除的 task deadline 链 | 后续协议能力继续落在 view/Point metadata 与标准协议 handler，不恢复旁路任务 API | 2026-08-10 |
| SF-AR-008 | 当前 Whale view-driven Starfish | DB 查询下推与最小 loader 校验边界 | AR | 高 | `starfish.adapters.db_views`、composition、protocol backends | P1 | 已完成（定向验证） | `connection_ids is None` 时 SQL 无 `WHERE`，非 `None` 使用 SQLAlchemy expanding bind `IN`；空列表查询前报错，缺失请求 ID 查询后报错；过滤与稳定顺序全部由 SQL `WHERE`/`ORDER BY` 完成，协议点位以 `vw_src_point_table.point_table_id` JOIN 取得 connection；loaders 在字段映射外不做 pandas 后过滤/排序、角色/协议归一化、协议有效性或 Point 覆盖校验 | unit + architecture 52 passed；真实 native integration 6 passed/0 skipped；数据库只读证据 `2026-08-10T02:03:26.106069Z`：IEC104 2 connections/6 points、ADS 1 connection/4 points；静态门禁通过 | 未跑全仓、性能或长稳；各 loader 仍为独立短只读事务，无 single snapshot | 查询选择、关联和顺序继续留在 SQL adapter；composition 负责跨帧基数，backend 负责协议角色约束 | 2026-08-10 |

## 7. 实现文件清单

```text
src/starfish/
├── __init__.py                        — 包入口，声明 runtime 核心定位与硬边界
├── __main__.py                        — CLI 入口（validate-plan / describe / health / read / run）
├── README.md                          — Starfish 模块职责、分层和阅读顺序说明
├── api/
│   ├── __init__.py                   — 高层 API 导出入口
│   └── runtime_api.py                — `StarfishRuntimeApi` 与默认装配入口
├── application/
│   ├── __init__.py                   — 应用层导出入口
│   └── runtime_service.py            — usecase 编排：plan 加载、driver 装配、runtime 创建
├── domain/
│   ├── __init__.py                   — 领域契约导出入口
│   ├── runtime.py                    — `StarfishRuntime` / `DriverEntry` 等核心抽象
│   └── server_plan.py                — ServerPlan 契约、校验结果和 `UnsupportedOperation`
├── drivers/
│   ├── __init__.py                   — drivers 包入口
│   ├── server_plan_loader.py         — ServerPlan 文件加载与校验
│   ├── runtime_registry.py           — 协议 driver 注册、dispatch 与 DriverEntry 装配
│   ├── server_simulator_facade.py    — 通用 in-memory stub driver
│   ├── http_rest_facade.py           — HTTP REST driver
│   ├── modbus_tcp_facade.py          — Modbus TCP driver
│   ├── mqtt_facade.py                — MQTT lightweight driver
│   ├── opcua_facade.py               — OPC UA native runner driver
│   ├── iec104_facade.py              — IEC104 native runner driver
│   ├── iec61850_mms_facade.py        — IEC61850 MMS native runner driver
│   ├── iec61850_report_facade.py     — IEC61850 Report driver
│   ├── iec101_facade.py              — IEC101 runtime facade 与 codec mode 诊断
│   ├── modbus_rtu_facade.py          — Modbus RTU PTY-backed driver
│   ├── ads_facade.py                 — ADS codebase-pending driver
│   ├── goose_facade.py               — GOOSE environment-pending driver
│   ├── sv_facade.py                  — SV environment-pending driver
│   └── native_runtime.py             — native runner 环境探测与共享辅助
├── native/
│   ├── __init__.py                   — native 支撑包入口
│   ├── CMakeLists.txt                — native runner 构建脚本
│   ├── README.md                     — native runner 构建与依赖说明
│   ├── process_handle.py             — 子进程生命周期管理
│   ├── runner_probe.py               — native binary 探测
...
└── protocols/
    ├── __init__.py                   — 协议工具包入口
    ├── iec101/                       — IEC101 编解码、信息对象、时标与链路层骨架
    └── modbus/
        ├── __init__.py               — Modbus register_encoding 导出入口
        └── register_encoding.py      — Modbus 寄存器编解码工具
```

```text
src/whale/ingest/diagnostics/
├── __init__.py                       — runtime 诊断工具导出入口
├── probe.py                          — 最小启动-健康-读取探测
...
├── test_probe_profile_capacity.py    — ingest diagnostics 对 starfish runtime 的 probe/profile/capacity 测试（~41 tests：覆盖 12 协议；real mode PASS + unavailable/pending/codec-only NOT_RUN + reason）
...
├── test_starfish_cli.py              — CLI（~30 tests：validate-plan/describe/health/read/run + per-endpoint mode 输出/--help/边界）
...
| 迁移项 | 旧路径 | 新路径 |
|---|---|---|
| OPC_UA C binary | `tools/source_lab/native/build/open62541_simulator_server` | `src/starfish/infrastructure/native/bin/open62541_simulator_server` |
| IEC104 C binary | `tools/source_lab/native/build/iec104_simulator_server` | `src/starfish/infrastructure/native/bin/iec104_simulator_server` |
| IEC61850 MMS C binary | `tools/source_lab/native/build/iec61850_simulator_server` | `src/starfish/infrastructure/native/bin/iec61850_simulator_server` |
| IEC61850 Report C binary | `tools/source_lab/native/build/iec61850_report_runner` | `src/starfish/infrastructure/native/bin/iec61850_report_runner` |
| MODBUS_TCP C binary | `tools/source_lab/native/build/modbus_simulator_server` | `src/starfish/infrastructure/native/bin/modbus_simulator_server` |
