...
| ADR-20260602-014-speed-layer-flink-pipeline-runtime-local-dev.md | Accepted | speed_layer Flink pipeline runtime + local dev runner | 只记录 LocalPipelineRunner 用于开发测试、FlinkPipelineAdapter 用于生产、writers/metrics 分层写入 |
| ADR-20260602-015-storage-three-layer-raw-archive-index-standardized.md | Accepted | storage 三层分层：raw_archive/raw_index/standardized | 只记录 raw_archive 用压缩文件（HDFS/Object Storage）、raw_index/standardized 用 TDengine（可替换）、warehouse/mart 当前为端口+stub |
