...
0. 普通会话中的代码修改由当前主会话完成编码阶段；不自动执行测试、验证或全量测试，不启动、委派或切换到独立 agent。
1. 优先清晰、直接、可读。
2. 保持当前模块、语言和框架的局部风格。
...
12. 修改代码必须保持类型、注释、配置和 schema 契约一致；测试资产的新增、修改和执行留给用户显式触发的 `/test` 阶段。
...
编码阶段不自动新增、修改或执行测试。用户显式调用 `/test` 后，应根据 staged diff 直接修改的行为评估和更新测试、fixture、fake/mock/stub 及必要索引：
...
测试阶段发现生产代码缺陷时必须停止并返回编码阶段，不得为满足旧断言而降低断言或混改生产代码。

## 6. 多语言质量

1. 遵循仓库配置的 lint、format、type-check、test 工具。
2. 语法/编译检查、lint/static analysis、type-check 和测试只在用户显式调用 `/test`、`/validate` 或 `/test-all` 时按对应阶段执行。
...
2. `project-tree-update` 和 `project-tree-reset` 仅在用户明确要求时由当前主会话执行，文件变化不自动触发目录树检查。
3. 状态只能基于当前真实证据更新。
