...
0. 任务等级 `light` / `standard` / `full` 是最高优先级第一步，必须在动手前判断清楚。判断规则参见 [CLAUDE.md §4](../CLAUDE.md) 与 [routing.md](routing.md) 的 `task_tier 读取策略` 段；命中白名单时由 `standard` 自动升级为 `full`。
1. 优先清晰、直接、可读。
2. 保持当前模块、语言和框架的局部风格。
...
2. `project-tree-update` 和 `project-tree-reset` 仅在用户明确要求时手动执行；handoff 必须转述该请求，文件变化不自动触发目录树检查。
3. 状态只能基于当前真实证据更新。
