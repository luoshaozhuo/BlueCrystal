...
def _list_fake_connection_ids(db_url: str) -> list[int]:
    """返回全量 selector 使用的 fake connection IDs。"""
...
def test_run_uses_whale_db_url(
    ...
    monkeypatch.setattr(starfish_cli, "build_server_manager_from_db", _build_fake_manager)
    monkeypatch.setattr(starfish_cli, "list_connection_ids_from_db", _list_fake_connection_ids)
