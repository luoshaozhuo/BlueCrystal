...
class TracePolicy:
    ...
    def __post_init__(self):
        if not 0 <= self.normal_sample_rate <= 1: raise ValueError("normal_sample_rate must be in [0,1]")
        if self.error_dedup_ttl_seconds <= 0: raise ValueError("error_dedup_ttl_seconds must be > 0")
        if self.error_dedup_max_entries <= 0: raise ValueError("error_dedup_max_entries must be > 0")
