...
static const uint16_t GOOSE_ETHERTYPE = 0x88b8;
...
static int open_capture_socket(const char *interface_id) {
    ...
    if (setsockopt(fd, SOL_PACKET, PACKET_ADD_MEMBERSHIP, &membership, sizeof(membership)) != 0) {
        /* Continue without ALLMULTI if the platform doesn't support it. */
    }
...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        printf("iec61850_goose_subscriber_runner\tversion=2\n");
        return 0;
    }
    ...
    while (monotonic_now_ms() < end_ms) {
        ...
        if (!parse_frame_app_id(frame, received, GOOSE_ETHERTYPE, &observed_app_id)) {
            continue;
        }
        ...
        g_msg_count++;
        printf("NOTIFY\t%u\t-\t-\t-\t0\t%d\t%lld\t1\t0\n",
               (unsigned int)observed_app_id,
               g_msg_count,
